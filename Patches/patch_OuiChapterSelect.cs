...
namespace Celeste {
    class patch_OuiChapterSelect : OuiChapterSelect {
        ...
        // Expose private fields to our mod
        [MonoModIgnore]
        ...
        public override bool IsStart(Overworld overworld, Overworld.StartMode start) {
            if (start == Overworld.StartMode.AreaComplete || start == Overworld.StartMode.AreaQuit) {
                AreaData area = AreaData.Get(SaveData.Instance.LastArea_Safe.ID);
                area = AreaData.Get(area?.Meta?.Parent) ?? area;
                if (area != null)
                    SaveData.Instance.LastArea_Safe.ID = area.ID;
            }
        ...
        public override IEnumerator Enter(Oui from) {
            ...
            currentLevelSet = SaveData.Instance?.LevelSet ?? "Celeste";
            ...
            if (from is OuiChapterPanel) {
                if (areaUnclamp >= 0 && areaUnclamp < icons.Count) {
            ...
            foreach (OuiChapterSelectIcon icon in icons) {
                AreaData areaData = AreaData.Get(icon.Area);
                if (areaData == null || areaData.LevelSet != currentLevelSet)
                    continue;

                int index = areaData.ToKey().ID;
        ...
        private void updateScarf() {
            try {
                if (AreaData.Areas?.Count == 0 || area < 0 || area >= (AreaData.Areas?.Count ?? 0))
                    return;

                AreaData areaData = AreaData.Areas[area];
                if (areaData == null || string.IsNullOrEmpty(areaData.Name))
                    return;

                string nextScarf = "areas/" + areaData.Name.ToLowerInvariant() + "_hover";
                if (levelSetScarf == null)
                    return;

                if (scarf == null || !nextScarf.Equals(scarf.AtlasPath)) {
                ...
                Logger.Log(LogLevel.Warn, "MaggyHelper", $"Error in updateScarf: {ex.Message}");
            }
        }
        ...
        public override void Render() {
            ...
            if (maplistEase > 0f) {
                ...
                (Input.GuiInputController(Input.PrefixMode.Latest) ? Input.GuiButton(Input.Pause) : Input.GuiButton(Input.ESC)).Draw(pos, Vector2.Zero, Color.White * Ease.CubeOut(maplistEase));
            }

            if (searchEase > 0f) {
                Vector2 pos = new Vector2(128f * Ease.CubeOut(searchEase), 952f);
                if (journalEnabled) {
                    pos.Y -= 256f;
                } else {
                    pos.Y -= 128f;
            ...
            if (levelsetEase > 0f) {
                Vector2 pos = new Vector2(1920f - 64f * Ease.CubeOut(maplistEase), 952f);
                string line = Dialog.CleanLevelSet(currentLevelSet);
